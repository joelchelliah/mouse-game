...
const jumpController = new JumpController({
  jumpSpeed: CAT_JUMP_SPEED,
  jumpForce: CAT_JUMP_FORCE,
  gravity: CAT_GRAVITY,
  cooldownDuration: CAT_JUMP_COOLDOWN,
});

const animationSequencer = new AnimationSequencer({
  alternateThreshold: IDLE_ALTERNATE_ANIMATIONS_THRESHOLD,
  alternatePlayCount: 2,
});

const movementController = new MovementController({
  runSpeed: CAT_RUN_SPEED,
  walkSpeed: CAT_WALK_SPEED,
  runThreshold: CAT_RUN_THRESHOLD,
  walkThreshold: CAT_WALK_THRESHOLD,
  stopThreshold: CAT_STOP_THRESHOLD,
});
...
export function update(starX, starY, active) {
  ...
  // Update jump cooldown timer every frame
  jumpController.updateCooldown();
  ...
  if (jumpController.isOnGround()) {
    ...
    if (movement.dx !== 0 || movement.dy !== 0) {
      ...
      y = movementController.clampY(y, grassTop(), window.innerHeight);
      pos.x = x;
      pos.y = y;
    }
  }
  ...
  const cooldownJustExpired = jumpController.cooldownJustExpired() && isStopped;
  ...
  if (jumpController.isOnGround()) {
    ...
    const jumpDirection = jumpController.getJumpDirection();
    container.scale.x =
      jumpDirection < 0
  ...
  if (jumpController.isJumping()) {
    const physics = jumpController.updatePhysics(depthScale);

    x += physics.dx;
    y += physics.dy;

    // Check if landed and clamp to ground level
    if (jumpController.checkLanding(y)) {
      y = jumpController.getGroundLevel();
      // Don't reset state yet - we need to play the landing frame first
    }
  ...
  const isMoving = movementController.getIsMoving();
  const movement = movementController.calculateMovement(
    { x, y },
    { x: starX, y: starY },
    effectiveDist,
  ...
  let desiredSprite;
  if (jumpController.state === "jump") {
    desiredSprite = "jump";
  } else if (jumpController.state === "fall") {
    desiredSprite = "fall";
  } else if (isMoving) {
    desiredSprite = movement.speed === "run" ? "run" : "walk";
  } else {
    // Idle - use animation sequencer to determine sprite
    const idleSprite = animationSequencer.getCurrentIdleSprite();
    desiredSprite = idleSprite ?? currentSpriteName;
  }
  ...
  const hasLanded = jumpController.checkLanding(y);
  if (
    jumpController.state === "fall" &&
    hasLanded &&
  ...
  if (++frameTick >= ticksPerFrame) {
    ...
    if (jumpController.state === "jump") {
      ...
      // Stay on last frame until state changes to "fall"
    }
    // Handle fall animation: hold on second-last frame until landing
      ...
      if (!hasLanded && currentFrame < totalFrames - 2) {
        ...
        jumpController.completeLanding();
        animationSequencer.reset();
      }
    }
    // Normal animation advancement for other states
      ...
      if (!isMoving && nextFrame === 0) {
        const newSprite = animationSequencer.onCycleComplete(totalFrames);
        if (newSprite) {
          setSprite(newSprite);
        }
      }
